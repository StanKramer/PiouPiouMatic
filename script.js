/* ================================================================
   🔹 PiouPiouMatic RP - Système Médical Interactif
   🔹 Section 1 : Structure, Zones, Hotspots, Positionnement
================================================================ */
...
const zones = [
  ...
  // --- MEMBRES SUPÉRIEURS (FACE)
  {id:"shoulder-left",x:21,y:22,name:"Épaule gauche (face)",tags:["shoulder","joint"]},
  {id:"shoulder-right",x:29,y:22,name:"Épaule droite (face)",tags:["shoulder","joint"]},
  ...
  {id:"scapula-left",x:72,y:26,name:"Omoplate gauche",tags:["shoulder","joint","back"]},
  {id:"scapula-right",x:78,y:26,name:"Omoplate droite",tags:["shoulder","joint","back"]},
  ...
  {id:"popliteal-left",x:73,y:70,name:"Creux poplité gauche",tags:["knee","joint","back"]},
  {id:"popliteal-right",x:77,y:70,name:"Creux poplité droit",tags:["knee","joint","back"]},
...
/* ================================================================
   🔹 HOTSPOTS : création + recalcul responsive
================================================================ */
...
/* Réagit au zoom, redimensionnement et scroll */
...
/* ================================================================
   🔹 PiouPiouMatic RP - Section 2
   🔹 Diagnostic, symptômes, traitements, procédures
================================================================ */
...
/* ---------- Fonctions utilitaires pour région anatomique ---------- */
function getZoneById(id) {
  return zones.find(z => z.id === id) || null;
}
function zoneHasTag(zone, tag) {
  return zone && zone.tags && zone.tags.includes(tag);
}
...
/* ---------- Application de modifieurs contextuels ---------- */
function applyModifiers(zone, type, pain, symptoms, treatments) {
  if (pain >= 8 && !treatments.includes("Morphine / monitorer la douleur"))
    treatments.push("Morphine / monitorer la douleur");
  else if (pain >= 6 && !treatments.includes("Antalgiques"))
    treatments.push("Antalgiques");

  if (zoneHasTag(zone,"eye")) {
    symptoms.push("Atteinte oculaire possible");
    treatments.push("Protection oculaire stérile");
  }
  ...
  if (zoneHasTag(zone,"joint") && type==="entorse")
    treatments.push("RICE (Repos, Glace, Compression, Élévation)","Immobilisation courte");
  if (zoneHasTag(zone,"joint") && type==="luxation")
    treatments.push("Réduction (médicale)","Immobilisation stricte");
  if (zoneHasTag(zone,"hand")||zoneHasTag(zone,"fingers"))
    treatments.push("Rééducation fonctionnelle (main)");
  if (zoneHasTag(zone,"tendon"))
    treatments.push("Échographie / immobilisation");
}
...
function composeDiagnosis(zone,type,pain){
  ...
  const additions = [];
  switch(type){
    case "arme feu": additions.push("plaie balistique","risque lésion profonde"); break;
    case "fracture": additions.push("déformation","mobilité douloureuse"); break;
    case "brulure": additions.push("évaluer le degré","surveillance infection"); break;
    case "entorse": additions.push("atteinte ligamentaire probable"); break;
  }
  if (zoneHasTag(zone,"eye")) additions.push("atteinte oculaire");
  if (zoneHasTag(zone,"jaw")) additions.push("douleur mastication");
  if (zoneHasTag(zone,"thorax")) additions.push("détresse respiratoire possible");
  if (zoneHasTag(zone,"abdomen")) additions.push("hémorragie interne possible");
  if (zoneHasTag(zone,"back")) additions.push("douleur paravertébrale");
  if (pain>=8) additions.push("douleur sévère");
  else if (pain>=6) additions.push("douleur modérée");
  return `${dx} — ${additions.join(", ")}`;
}
...
/* ---------- Génération des procédures détaillées ---------- */
function generateProcedures(zone,type,pain){
  ...
  switch(type){
    case"arme feu":add(
      "Ne pas retirer le projectile.",
      "Compression hémorragique.",
      "Voie veineuse + antibiotiques IV.",
      "Scanner de bilan trajectoire.",
    ...
    case"brulure":add(
      "Refroidir eau tempérée 10 min.",
      "Pansement gras stérile.",
      "Évaluer surface et degré.",
      "Contrôle 24–48 h."
    );break;
    case"entorse":add(
      "Protocole RICE.",
      "Immobilisation orthèse 3 sem.",
      "Kiné proprioception."
    );break;
  }
...
/* ---------- Moteur principal de mise à jour du diagnostic ---------- */
...
/* ================================================================
   🔹 PiouPiouMatic RP – Section 3
   🔹 Easter Eggs, Code Bleu, Modale, Gestion des blessures
================================================================ */
...
function handleZoneClick(zone, e) {
  ...
  // 🍆 Stan (10 clics sur l’entrejambe)
  if (zone.id === "groin") {
    groinClicks++;
    if (groinClicks === 10) {
      activateSoundSystem();
...
function triggerCodeBlue() {
  ...
  if (status) {
    ...
    status.innerHTML =
      "🩸 <strong>Code Bleu activé — Patient en détresse critique !</strong>";
    setTimeout(() => {
...
function renderInjuries() {
  const groups = {};
  injuries.forEach((i) => {
    const z = getZoneById(i.zoneId) || { tags: [] };
...
function toggleCalibration() {
  ...
  const existing = document.querySelectorAll(".calib-label");
